...
class EmbeddingManager(nn.Module):
    def __init__(self, embedder, placeholder_strings=None, initializer_words=None, per_image_tokens=False, num_vectors_per_token=1, progressive_words=False, 
                 **kwargs):
        super().__init__()
        ...
        for idx, placeholder_string in enumerate(placeholder_strings):
            ...
            if initializer_words and idx < len(initializer_words):
                if len(initializer_words[idx]) > 0:
                    init_word_token = get_token_for_string(initializer_words[idx])
                    with torch.no_grad():
                        init_word_embedding = get_embedding_for_tkn(init_word_token) # (..cpu()) - fixed in ddpm
                    token_params = torch.nn.Parameter(init_word_embedding.unsqueeze(0).repeat(num_vectors_per_token, 1), requires_grad=True)
                    self.initial_embeddings[placeholder_string] = torch.nn.Parameter(init_word_embedding.unsqueeze(0).repeat(num_vectors_per_token, 1), 
                                                                                     requires_grad=False)
                    print('.. embeddings init .. token:', placeholder_string, token, '.. term:', initializer_words[idx], init_word_token)
    ...
    def save(self, ckpt_path):
        torch.save({'string_to_token': self.string_to_token_dict, 'string_to_param': self.string_to_param_dict}, ckpt_path)
