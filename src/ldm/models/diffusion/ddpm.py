...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                ...
                unfreeze_model=False, # text invers !?
                model_lr=0., # text invers !?
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        print(f'.. model {path}', f':: keys {missing} missed, {unexpected} unexpected' if len(missing) > 0 or len(unexpected) > 0 else '')
    ...
    def training_step(self, batch, batch_idx):
        if self.ucg_training is not None: # !!! not in text inversion
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.uses_rml_inpainting = conditioning_key == 'hybrid' and self.model.diff_model_config['params']['in_channels'] == 9
    ...
    def create_embedding_manager(self):
        self.embedding_manager = self.instantiate_embedding_manager(self.personalization_config, self.cond_stage_model)
...
# !!! custom diff + text inversion
                try: # FrozenCLIPEmbedder [modules], textual inversion
                    if hasattr(self, 'embedding_manager'): # training
                ...
                except: # FrozenCLIPEmbedderWrapper [custom modules], custom diffusion
    ...
    @torch.no_grad()
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False, cond_key=None, return_original_cond=False, bs=None): # , return_x=False
    ...
    @torch.no_grad()
    def get_input_rml_inpainting(self, batch, bs=None, return_first_stage_outputs=False):
        # note: restricted to non-trainable encoders currently
        assert (not self.cond_stage_trainable), "trainable cond stages not yet supported for inpainting"
        z, c, x, xrec, xc = self.get_input(batch, self.first_stage_key, return_first_stage_outputs=True, force_c_encode=True, return_original_cond=True, bs=bs)
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        if hasattr(self, 'split_input_params'):
            ...
            if isinstance(self.first_stage_model, VQModelInterface):
                ...
                return self.first_stage_model.decode(z) # this
    ...
    def shared_step(self, batch, **kwargs):
        if self.uses_rml_inpainting is True:
            x, c = self.get_input_rml_inpainting(batch, self.first_stage_key)
        else: 
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, 
                        return_codebook_ids=False, quantize_denoised=False, return_x0=False, score_corrector=None, corrector_kwargs=None):
        t_in = t
    ...
    @torch.no_grad()
    def p_sample(self, x, c, t, clip_denoised=False, repeat_noise=False, return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, c=c, t=t, clip_denoised=clip_denoised, return_codebook_ids=return_codebook_ids, quantize_denoised=quantize_denoised,
                                       return_x0=return_x0, score_corrector=score_corrector, corrector_kwargs=corrector_kwargs)
        if return_codebook_ids:
    ...
    @torch.no_grad()
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False, img_callback=None, mask=None, x0=None, 
                temperature=1.0, noise_dropout=0.0, score_corrector=None, corrector_kwargs=None, batch_size=None, x_T=None, start_T=None, log_every_t=None):
        if not log_every_t:
    ...
    @torch.no_grad()
    def p_sample_loop(self, cond, shape, return_intermediates=False, x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
    ...
    @torch.no_grad()
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None, verbose=True, timesteps=None, quantize_denoised=False, 
        ...
        if cond is not None:
            if isinstance(cond, dict):
                ...
                cond = [c[:batch_size] for c in cond] if isinstance(cond, list) else cond[:batch_size]
        return self.p_sample_loop(cond, shape, return_intermediates=return_intermediates, x_T=x_T, verbose=verbose, timesteps=timesteps, 
    ...
    @torch.no_grad()
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=1., return_keys=None, quantize_denoised=True, inpaint=False,
                   plot_denoise_rows=False, plot_progressive_rows=False, plot_diffusion_rows=False, **kwargs):

        use_ddim = ddim_steps is not None

        log = dict()
        get_input_fn = self.get_input_rml_inpainting if self.uses_rml_inpainting is True else self.get_input
        z, c, x, xrec, xc = get_input_fn(batch, self.first_stage_key, return_first_stage_outputs=True, force_c_encode=True, return_original_cond=True, bs=N)
        N = min(x.shape[0], N)
        n_row = min(x.shape[0], n_row)
        log['inputs'] = x
        ...
        if sample:
            ...
            sample_scaled, _ = self.sample_log(cond=c, batch_size=N, ddim=use_ddim, ddim_steps=ddim_steps, eta=ddim_eta,
...
# !!! text inversion
        if hasattr(self, 'embedding_manager') and self.embedding_manager is not None:
            ...
            if self.unfreeze_model: # train base model? => two different param groups
                model_params = list(self.cond_stage_model.parameters()) + list(self.model.parameters())
                params = [{"params": params, "lr": lr}, {"params": model_params}]
                lr = self.model_lr
        else:
            params = list(self.model.parameters())
            if self.cond_stage_trainable:
...
# !!! for optims swapping in text inversion training
    ...
    def on_save_checkpoint(self, checkpoint):
        if hasattr(self, 'embedding_manager'): # !!! save only embedding_manager, if it exists [that means not custom diffusion]
            if not self.unfreeze_model: # not tuning the model => zero-out checkpoint content
                checkpoint.clear()
            if os.path.isdir(self.trainer.checkpoint_callback.dirpath):
                ...
                self.embedding_manager.save(os.path.join(self.trainer.checkpoint_callback.dirpath, f'embeddings-{self.global_step}.pt'))
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None):
        if self.conditioning_key is None:
            ...
            cc = torch.cat(c_crossattn, 1) if not self.sequential_cross_attn else c_crossattn
            out = self.diffusion_model(x, t, context=cc)
        elif self.conditioning_key == 'hybrid':
            xc = torch.cat([x] + c_concat, dim=1) # [2, 9, 64, 64]
            cc = torch.cat(c_crossattn, 1) # [4, 77, 768]
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=100, ddim_eta=1., return_keys=None,
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            with ema_scope("Sampling with classifier-free guidance"):
                samples_cfg, _ = self.sample_log(cond=c, batch_size=N, ddim=use_ddim, ddim_steps=ddim_steps, eta=ddim_eta,
                                                 unconditional_guidance_scale=unconditional_guidance_scale, unconditional_conditioning=uc)
...
class LatentFinetuneDiffusion(LatentDiffusion):
    ...
    """
    def __init__(self, concat_keys: tuple, 
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        if len(missing) > 0 or len(unexpected) > 0:
            print(f'Restored {path} :: keys {missing} missed, {unexpected} unexpected')
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            with ema_scope("Sampling with classifier-free guidance"):
                samples_cfg, _ = self.sample_log(cond={"c_concat": [c_cat], "c_crossattn": [c]},
                                                 batch_size=N, ddim=use_ddim, ddim_steps=ddim_steps, eta=ddim_eta,
                                                 unconditional_guidance_scale=unconditional_guidance_scale, unconditional_conditioning=uc_full)
                x_samples_cfg = self.decode_first_stage(samples_cfg)
                log[f"samples_cfg_scale_{unconditional_guidance_scale:.2f}"] = x_samples_cfg
